...
/* package */ class DecideChecker {
    ...
    private Result runDecideCheck(final String token, final String distinctId, final RemoteService poster)
        ...
        MPLog.v(LOGTAG, "Mmp decide server response was:\n" + responseString);
    ...
    private static List<InAppNotification> parseInAppNotifications(JSONObject response) {
        ...
        if (response.has(NOTIFICATIONS)) {
            try {
                ...
                MPLog.e(LOGTAG, "Mmp endpoint returned non-array JSON for " + NOTIFICATIONS + ": " + response);
            }
        }
    ...
    /* package */ static Result parseDecideResponse(String responseString)
        ...
        if (response.has(EVENT_BINDINGS)) {
            try {
                ...
                MPLog.e(LOGTAG, "Mmp endpoint returned non-array JSON for event bindings: " + response);
            }
        }
        ...
        if (response.has(VARIANTS)) {
            try {
                ...
                MPLog.e(LOGTAG, "Mmp endpoint returned non-array JSON for variants: " + response);
            }
        }
        ...
        if (response.has(AUTOMATIC_EVENTS)) {
            try {
                ...
                MPLog.e(LOGTAG, "Mmp endpoint returned a non boolean value for automatic events: " + response);
            }
        }
        ...
        if (response.has(INTEGRATIONS)) {
            try {
                ...
                MPLog.e(LOGTAG, "Mmp endpoint returned a non-array JSON for integrations: " + response);
            }
        }
